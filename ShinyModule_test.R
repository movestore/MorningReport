...
# tabsetPanel(
  tabPanel("Settings", 
           fluidRow(
             column(2,dateInput(ns("date"),"Date:", value = Sys.Date())),
             column(2,timeInput(ns("time"), "Time (24h format):", value = Sys.time(),seconds = FALSE))
             ),
           # fluidRow(
           ...
           bsTooltip(id=ns("mig7d_dist"), title="Define the radius (in m) an animal must move past to qualify as migration behaviour. E.g. 100000m (100 km)", placement = "bottom", trigger = "hover", options = list(container = "body")),
           numericInput(ns("dead7d_dist"),"Mortality buffer in m (last 7 days", min=0, value=NA),
           bsTooltip(id=ns("dead7d_dist"), title="Define the radius (in m) within which an animal must remain to indicate a likely mortality. E.g. 100 m", placement = "bottom", trigger = "hover", options = list(container = "body"))
  ),
...
shinyModule <- function(input, output, session, data){#, time_now=NULL, posi_lon=NULL, posi_lat=NULL, mig7d_dist, dead7d_dist) {
  ...
  output$start_timeUI <- renderUI({
    sliderInput(ns("start_time"),
                ...
                max = as.POSIXct(paste0(input$date," ",strftime(input$time, format="%H:%M:%S"))),
